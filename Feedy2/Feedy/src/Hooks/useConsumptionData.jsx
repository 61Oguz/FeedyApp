...
const useConsumptionData = (userId) => {
  ...
  const [yesterdayConsumption, setYesterdayConsumption] = useState({
    calories: 0,
    protein: 0,
    carbs: 0,
    fat: 0,
  ...
  const fetchTodayConsumption = async () => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:8080/api/foodlog/${userId}/${todayDate}`,
      );
  ...
  const fetchYesterdayConsumption = async () => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:8080/api/foodlog/${userId}/${formattedYesterdayDate}`,
      );
      const { calories, protein, carbs, fat } = response.data.reduce(
        (totals, log) => {
          totals.calories += log.calories;
