...
const FoodLogPage = () => {
  ...
  const [datesConsumption, setDatesConsumption] = useState({
    calories: 0,
    protein: 0,
    carbs: 0,
    fat: 0,
  ...
  const fetchFoodLogs = async () => {
    try {
      const response = await axios.get(
        `http://localhost:8080/api/foodlog/${user.userId}/${date}`,
      );
  ...
  const fetchDatesConsumption = async () => {
    try {
      ...
      const { calories, protein, carbs, fat } = response.data.reduce(
        (totals, log) => {
  ...
  const handleSuggestionsFetchRequested = async ({ value }) => {
    try {
      const response = await axios.get(
        `https://api.nal.usda.gov/fdc/v1/foods/search?api_key=uDkRCNEx1og7hBgeppz9TWfdcteGAG6PWynpd3iA&query=${value}`,
      );
      const uniqueFoods = [];
    ...
    <div>
      <div className={`TodoWrapper ${isSidebarOpen ? "" : "sidebar-closed"}`}>
        ...
        <div id={"doughnutChart"} className={"DoughnutChartWrapper"}>
          <Doughnut
            data={doughnutData}
            options={doughnutOptions("Calories")}
