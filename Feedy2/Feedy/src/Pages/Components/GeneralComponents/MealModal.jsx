...
const MealModal = ({ isOpen, onClose, meals, dailyCalorieNeed }) => {
    ...
    // Function to download the meal list as PDF
    const handleDownloadPDF = async () => {
        const pdf = new jsPDF();
        const modalCanvas = await html2canvas(modalRef.current);
        const modalImage = modalCanvas.toDataURL('image/png');

        pdf.addImage(modalImage, 'PNG', 10, 10, 190, 210); // Adjust as needed
        pdf.save('meal-list.pdf');
    };

    // Function to sum up all macro values
    const getTotalMacros = (mealItems) => {
        return mealItems.reduce((totals, food) => {
    ...
    return (
        <div className="modal-overlay">
            <div className="modal-content" ref={modalRef}>
                ...
                <p className="modal-headers">Based on your daily calorie need of {dailyCalorieNeed} kcal:</p>
                    ...
                    <div key={index} className="meal-section">
                        ...
                        <ul>
                            {meal.items.map((food, foodIndex) => (
                                <li key={foodIndex}>
                                    {food.name} - {food.calories} kcal (Protein: {food.protein}g, Fat: {food.fat}g, Carbs: {food.carbs}g)
                                </li>
                            ))}
                        </ul>
                    </div>
                ...
                <button onClick={handleDownloadPDF} className="pdf-download-button-meal">
                    Download Meal Plan as PDF
                </button>
                <button onClick={onClose} className="close-button-meal">Close</button>
            </div>
        </div>
    );
};
